...
with st.sidebar:
    ...
    if api_key:
        ...
        st.warning("⚠️ Please enter API key to use stock features")
...
try:
    ...
    finbert = None
    st.warning("⚠️ Sentiment model not loaded. Install transformers: pip install transformers torch")
...
# ===========================
with tab1:
    st.header("📰 Market News with Sentiment Analysis")
        ...
        if st.button("🔄 Refresh News", type="primary", use_container_width=True):
            with st.spinner("Fetching latest news..."):
                ...
                if articles and finbert:
                    ...
                    st.success(f"✅ Loaded {len(st.session_state.news_articles)} articles")
                    st.rerun()

    with col2:
        if st.button("🗑️ Clear News", use_container_width=True):
            st.session_state.news_articles = []
            st.rerun()
    ...
    if st.session_state.news_articles:
        ...
        st.info("👆 Click 'Refresh News' to load market news")
...
with tab2:
    ...
    if not st.session_state.api_initialized:
        st.warning("⚠️ Please enter your Alpha Vantage API key in the sidebar")
    else:
        ...
        with st.spinner(f"Fetching data for {selected_stock}..."):
            ...
            if quote:
                ...
                with col2:
                    ...
                    st.metric("Change", f"{change_pct:+.2f}%", delta=f"₹{quote['change']:.2f}")
                with col3:
                ...
                if not df_prices.empty:
                    ...
                    with col1:
                        st.metric("30-Day High", f"₹{df_prices['high'].max():.2f}")
                    with col2:
                        st.metric("30-Day Low", f"₹{df_prices['low'].min():.2f}")
                    with col3:
                ...
                st.error("❌ Failed to fetch stock data. Check API key or symbol.")
...
with tab4:
    ...
    if not st.session_state.api_initialized:
        ...
        if st.button("📊 Compare", type="primary") and selected_stocks:
            comparison_data = []
...
# ===========================
st.markdown("---")
st.caption("💡 Market Intelligence Dashboard | Data from Alpha Vantage & Google News")
st.caption("⚠️ Free API tier: 25 requests/day | Rate limited: 12 seconds between requests")
